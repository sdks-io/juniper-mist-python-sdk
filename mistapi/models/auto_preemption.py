# -*- coding: utf-8 -*-

"""
mistapi

This file was automatically generated by APIMATIC v3.0 (
 https://www.apimatic.io ).
"""
from mistapi.api_helper import APIHelper


class AutoPreemption(object):

    """Implementation of the 'auto_preemption' model.

    schedule to preempt ap’s which are not connected to preferred peer

    Attributes:
        day_of_week (DayOfWeekEnum): TODO: type description here.
        enabled (bool): whether auto preemption should happen
        time_of_day (string): any / HH:MM (24-hour format)

    """

    # Create a mapping from Model property names to API property names
    _names = {
        "day_of_week": 'day_of_week',
        "enabled": 'enabled',
        "time_of_day": 'time_of_day'
    }

    _optionals = [
        'day_of_week',
        'enabled',
        'time_of_day',
    ]

    def __init__(self,
                 day_of_week=APIHelper.SKIP,
                 enabled=False,
                 time_of_day='any'):
        """Constructor for the AutoPreemption class"""

        # Initialize members of the class
        if day_of_week is not APIHelper.SKIP:
            self.day_of_week = day_of_week 
        self.enabled = enabled 
        self.time_of_day = time_of_day 

    @classmethod
    def from_dictionary(cls,
                        dictionary):
        """Creates an instance of this model from a dictionary

        Args:
            dictionary (dictionary): A dictionary representation of the object
            as obtained from the deserialization of the server's response. The
            keys MUST match property names in the API description.

        Returns:
            object: An instance of this structure class.

        """
        if dictionary is None:
            return None

        # Extract variables from the dictionary

        day_of_week = dictionary.get("day_of_week") if dictionary.get("day_of_week") else APIHelper.SKIP
        enabled = dictionary.get("enabled") if dictionary.get("enabled") else False
        time_of_day = dictionary.get("time_of_day") if dictionary.get("time_of_day") else 'any'
        # Return an object of this model
        return cls(day_of_week,
                   enabled,
                   time_of_day)

# -*- coding: utf-8 -*-

"""
mistapi

This file was automatically generated by APIMATIC v3.0 (
 https://www.apimatic.io ).
"""
from mistapi.models.anomaly import Anomaly


class AnomalyMetrics(object):

    """Implementation of the 'AnomalyMetrics' model.

    TODO: type model description here.

    Attributes:
        end (int): TODO: type description here.
        limit (int): TODO: type description here.
        page (int): TODO: type description here.
        results (list of Anomaly): TODO: type description here.
        start (int): TODO: type description here.

    """

    # Create a mapping from Model property names to API property names
    _names = {
        "end": 'end',
        "limit": 'limit',
        "page": 'page',
        "results": 'results',
        "start": 'start'
    }

    def __init__(self,
                 end=None,
                 limit=None,
                 page=None,
                 results=None,
                 start=None):
        """Constructor for the AnomalyMetrics class"""

        # Initialize members of the class
        self.end = end 
        self.limit = limit 
        self.page = page 
        self.results = results 
        self.start = start 

    @classmethod
    def from_dictionary(cls,
                        dictionary):
        """Creates an instance of this model from a dictionary

        Args:
            dictionary (dictionary): A dictionary representation of the object
            as obtained from the deserialization of the server's response. The
            keys MUST match property names in the API description.

        Returns:
            object: An instance of this structure class.

        """
        if dictionary is None:
            return None

        # Extract variables from the dictionary

        end = dictionary.get("end") if dictionary.get("end") else None
        limit = dictionary.get("limit") if dictionary.get("limit") else None
        page = dictionary.get("page") if dictionary.get("page") else None
        results = None
        if dictionary.get('results') is not None:
            results = [Anomaly.from_dictionary(x) for x in dictionary.get('results')]
        start = dictionary.get("start") if dictionary.get("start") else None
        # Return an object of this model
        return cls(end,
                   limit,
                   page,
                   results,
                   start)
